...
use crate::{
    db,
    db::links::{CreateLinkError, GetLinkError},
    state::AppState,
};
...
impl link_service_server::LinkService for LinkService {
    async fn create_link(&self, request: Request<CreateLinkRequest>) -> Result<Response<CreateLinkResponse>, Status> {
        ...
        db::links::create_link(&self.state.db_pool, short_code.clone(), target.clone())
            .await
        ...
        crate::cache::cache_link(self.state.redis.clone(), short_code.clone(), target).await;
    ...
    async fn get_link(&self, request: Request<GetLinkRequest>) -> Result<Response<GetLinkResponse>, Status> {
        ...
        crate::cache::cache_link(self.state.redis.clone(), short_code, target.clone()).await;
